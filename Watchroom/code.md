...
let
    ...
    FROM (
        ...
        -- Second part of the union (NFIRS)
        SELECT 
            'NFIRS' AS report,
    ...
    -- Apply STRING_SPLIT to break the 'Responding Units' column
    CROSS APPLY (
        SELECT 
            LTRIM(RTRIM(value)) AS unit -- Extract individual units from the delimited list
